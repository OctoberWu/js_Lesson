...
vis.Network.prototype.animateTraffic =
  function (edgesTrafficList, onPreAnimationHandler, onPreAnimateFrameHandler,
    onPostAnimateFrameHandler, onPostAnimationHandler) {
    ...
    var trafficAnimator = {
      ...
      getNetworkTrafficCanvas: function () {
        this.trafficCanvas = this.thisNetwork.body.container.getElementsByClassName('networkTrafficCanvas')[0];
        ...
        if (this.trafficCanvas == undefined) {
          ...
          this.trafficCanvas.width = frame.canvas.clientWidth;   // take the relative coordinates
          this.trafficCanvas.height = frame.canvas.clientHeight; // take the relative coordinates
...
vis.Network.prototype.animateTrafficOnPreAnimation = function (edgesTrafficList) {
  ...
  for (var i in edgesTrafficList) {
    ...
    const fromValue = edgeTraffic.edge.from.getValue()
    if (parseFloat(fromValue)) {
      const newValue = fromValue +
        (edgeTraffic.isBackward ? -1 : 1) * -edgeTraffic.trafficSize;

      this.thisNetwork.body.data.nodes.update({ id: edgeTraffic.edge.fromId, value: Math.max(0, newValue) });
    }
  }
};
